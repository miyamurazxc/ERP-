    ...
    <div class="shell">
        ...
        <aside class="sidebar">
            ...
            @auth
                ...
                <nav class="nav">
                    {{-- Журнал заявок показываем кадровику, директору и админу. --}}
                    @if (auth()->user()->canViewAllRequests())
                        <a href="{{ route('requests.index') }}" class="{{ request()->routeIs('requests.index') || request()->routeIs('requests.show') ? 'active' : '' }}">Журнал заявок</a>
                    @endif
                    {{-- Кнопки создания заявок показываем только тем, кто имеет право создавать заявки. --}}
                    @if (auth()->user()->canCreateRequests())
                        <a href="{{ route('requests.create') }}" class="{{ request()->routeIs('requests.create') ? 'active' : '' }}">Заявка</a>
                    @endif
